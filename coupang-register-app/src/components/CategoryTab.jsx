...
export default function CategoryTab({ category, isAdmin }) {
  ...
  useEffect(() => {
    fetch(`/api/coupang/register/templates?category=${catId}`, { headers: authHeader() })
      .then(r => r.ok ? r.json() : [])
      .then(list => {
        setTemplates(list);
      ...
      .catch(() => {});
    fetch(`/api/coupang/register/settings?category=${catId}`, { headers: authHeader() })
      .then(r => r.ok ? r.json() : {})
  ...
  async function handleSaveSettings() {
    ...
    setSettingsMsg('');
    try {
      const r = await fetch('/api/coupang/register/settings', {
        method: 'POST',
    ...
    <>
      ...
      <div className="card">
        ...
        {hasCSV ? (
          ...
          <div style={{ fontSize: 12, color: 'var(--text3)' }}>자동 조회 미지원 카테고리입니다</div>
        )}

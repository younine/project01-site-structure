...
export default function CandidateList({ onSelect }) {
  ...
  useEffect(() => {
    fetch('/api/coupang/register/candidates', { headers: authHeader() })
  ...
  if (loading) return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ color: 'var(--text3)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
  ...
  if (items.length === 0) return (
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-title">등록 대상 추천</div>
      <div style={{ color: 'var(--text3)', fontSize: 12 }}>미등록 상품이 없습니다.</div>
    </div>
  );
      ...
      {!collapsed && (
        <>
          ...
          {items.length > PAGE_SIZE && (
            <button
              className="btn btn-secondary"
              style={{ marginTop: 8, width: '100%', fontSize: 12, height: 30 }}

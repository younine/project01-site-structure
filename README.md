...
OCI 서버(168.110.111.1)에서 운영 중인 멀티 앱 구조. 각 앱은 React + Vite로 빌드되며 `/var/www/html` 아래에 정적 파일로 배포된다. API 서버는 `~/monitor-api/server.js` (Express, PM2 `monitor-api`, port 3000).
...
단일 빌드 파일(`/var/www/html/index.html`)로 6개 경로를 담당한다.
...
events는 로그인 필수이므로 `authLoading` 분기 이후에 처리.
...
~/main-app/src/
  App.jsx                       # 라우팅 진입점
  App.css                       # 전역 스타일 (dashboard + community + nrank + events 모든 CSS 포함)
  hooks/
  ...
  components/
    ...
    DailyTab.jsx                # 일별 랭킹 탭
    RealtimeTab.jsx             # 실시간 랭킹 탭
    EventsPage.jsx              # 행사 스케줄 (Calendar + PromoForm + PromoList + DetailPanel 감싸는 컨테이너)
...
`monitor-api`가 로그인 처리 및 JWT 발급. 토큰 만료 7일.
...
3. `monitor-api/server.js`의 `ALL_PERMISSIONS`에 추가
4. 모든 앱 재빌드

### useAuth 훅

`~/main-app/src/hooks/useAuth.js`
...
POST   /api/events/sync           # Notion 동기화 (admin)
...
[발주서 변환]
POST /api/compuzone/xls           # 컴퓨존 엑셀 파싱
GET  /api/order/hs-codes          # HS 코드 목록
POST /api/order/hs-codes          # HS 코드 등록
POST /api/order/xls               # 발주서 엑셀 변환
POST /api/order/miracle/download  # 미라클 발주서 다운로드
POST /api/order/atoz/download     # 아토즈 발주서 다운로드
POST /api/order/assacom/download  # 아싸컴 발주서 다운로드

[B2B]
GET  /api/b2b/settings            # B2B 설정
POST /api/b2b/settings            # B2B 설정 저장
GET  /api/b2b/atoz/sheet-cache    # 아토즈 시트 캐시
POST /api/b2b/parse               # B2B 파싱
...
| 이름 | 포트 | 경로 | 설명 |
|---|---|---|---|
| `monitor-api` | 3000 | `~/monitor-api/server.js` | 메인 API 서버 |
| `coupang-collector` | 3001 | `~/coupang-collector/server.js` | 쿠팡 가격 수집기 (별도 프로세스) |

정적 파일 배포(빌드) 후에는 PM2 재시작 불필요. `monitor-api/server.js` 수정 시에만 `pm2 restart monitor-api`.
...
/keyboard/         → try_files ... /keyboard/index.html
/b2b-order/        → try_files ... /b2b-order/index.html
...
/api/              → proxy_pass http://localhost:3000   (monitor-api)
/                  → try_files $uri $uri/ =404
```

주의: `/community/`, `/nrank/`, `/events/`, `/admin-settings/`는 별도 빌드 디렉토리 없이 모두 `/index.html`(main-app)로 폴백. 해당 경로 아래에 실제 디렉토리가 존재하면 nginx가 먼저 서빙하므로, 구 빌드 디렉토리가 남아있으면 main-app 폴백이 작동하지 않는다.
...
- PromoList: 등록된 행사 목록
- DetailPanel: 선택된 행사 상세 (슬라이드 패널). 상품코드/모델명/판매가/할인/최종혜택가/메모 표시. `enrichEvents()`가 서버사이드에서 products 데이터와 조인해 채널별 상품코드를 주입함

CSS 주의: `events-layout` 클래스로 2단 레이아웃 구성. main-app의 전역 `.content` 스타일과 충돌 방지를 위해 `.content.events-layout`으로 추가 클래스 방식 사용.
...
- RealtimeTab: 실시간 랭킹
- 랭킹 데이터는 GitHub `younine/naver-ranking` 레포의 CSV 파일. GitHub Actions로 수집. 설정값(`NRANK_CATEGORIES`, `NAVER_CLIENT_ID/SECRET`)은 `monitor-api/server.js`에 있음

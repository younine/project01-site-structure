...
const B2B_COLUMNS = [
  ...
  { key: 'salePrice',    label: '판매가',     width: 80 },
  { key: 'hsCode',       label: 'HS코드',     width: 80 },
  { key: 'atozPrice',    label: '아토즈단가', width: 85 },
  { key: 'assacomPrice', label: '아싸컴단가', width: 85 },
  { key: 'miraclePrice', label: '미라클단가', width: 85 },
];

const MARKET_COLUMNS = [
  { key: 'modelName',      label: '모델명',      width: 200 },
  { key: 'salePrice',      label: '판매가',      width: 90  },
  { key: 'skuId',          label: 'SKU ID',      width: 120 },
  { key: 'naverCode',      label: '네이버',      width: 120 },
  { key: 'catalogCode',    label: '카탈로그',    width: 120 },
  { key: 'gmarketCode',    label: '지마켓',      width: 120 },
  { key: 'auctionCode',    label: '옥션',        width: 120 },
  { key: 'gmarketAnyCode', label: '지마켓any',   width: 120 },
  { key: 'auctionAnyCode', label: '옥션any',     width: 120 },
  { key: 'elevenCode',     label: '11번가',      width: 120 },
  { key: 'himartCode',     label: '하이마트',    width: 120 },
  { key: 'lotteCode',      label: '롯데온',      width: 120 },
  { key: 'ssgCode',        label: 'SSG',         width: 120 },
  { key: 'aliCode',        label: '알리',        width: 120 },
  { key: 'odCode',         label: '오늘의집',    width: 120 },
  { key: 'kakaoCode',      label: '카카오',      width: 120 },
  { key: 'compuzoneCode',  label: '컴퓨존',      width: 120 },
  { key: 'coupangItemId',  label: '쿠팡아이템id', width: 130 },
];
...
export const EMPTY_ROW = () => ({
  ...
  skuId: '',
  naverCode: '', catalogCode: '',
  gmarketCode: '', auctionCode: '',
  gmarketAnyCode: '', auctionAnyCode: '',
  elevenCode: '', himartCode: '', lotteCode: '', ssgCode: '',
  aliCode: '', odCode: '', kakaoCode: '', compuzoneCode: '',
  coupangItemId: '',
});
...
export default function ProductSheet({ activeTab, rows, setRows, loading, saving, msg, onSave, hasPermission }) {
  ...
  const COLUMNS = activeTab === 'market' ? MARKET_COLUMNS : B2B_COLUMNS;
  const tabLabel = activeTab === 'market' ? '마켓 코드' : 'B2B단가표';

  const filteredRows = searchQuery.trim()
    ? (() => {
        const terms = searchQuery.split(/[\s,]+/).map(t => t.toLowerCase()).filter(Boolean);
        return rows.filter(r =>
          terms.some(t => (r.modelName || '').toLowerCase().includes(t))
        );
      })()
    : rows;
    ...
    const handleKeyDown = (e) => {
      ...
      for (let r = minRow; r <= maxRow; r++) {
        ...
        for (let c = minCol; c <= maxCol; c++) {
          ...
          const val = PRICE_KEYS.has(key) ? fmtPrice(filteredRows[r]?.[key]) : (filteredRows[r]?.[key] ?? '');
  ...
  const handleCellPaste = useCallback((e, rowIdx, colIdx) => {
    ...
    setRows(prev => {
      ...
      return activeTab === 'b2b' ? recalcPrices(next) : next;
    });
  }, [setRows, COLUMNS, activeTab]);
  ...
  const pasteHint = activeTab === 'market'
    ? '열 순서: 모델명 / 판매가 / SKU ID / 네이버 / 카탈로그 / 지마켓 / 옥션 / 지마켓any / 옥션any / 11번가 / 하이마트 / 롯데온 / SSG / 알리 / 오늘의집 / 카카오 / 컴퓨존 / 쿠팡아이템id'
    ...
    <div className="product-sheet">
      <div className="sheet-toolbar">
        <div className="sheet-toolbar-left">
          <span className="sheet-title">{tabLabel}</span>
          <span className="sheet-count">
            {searchQuery.trim() ? `${filteredRows.length} / ${rows.length}행` : `${rows.length}행`}
          </span>
          <div className="sheet-search-wrap">
            <input
              type="text"
              className="sheet-search"
              placeholder="모델명 검색 (스페이스·쉼표로 여러 개)"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
            />
            {searchQuery && (
              <button className="sheet-search-clear" onClick={() => setSearchQuery('')}>✕</button>
            )}
          </div>
          {msg && (
        ...
        <div className="sheet-toolbar-right">
          ...
          <button className="btn-sheet btn-export" onClick={exportExcel}>엑셀 내보내기</button>
          {isEditor && (
      ...
      {isEditor && (
        <div className="paste-hint">
          셀 드래그로 범위 선택 후 Ctrl+C: 탭·줄바꿈 형식 복사 (엑셀 붙여넣기 가능) |
          셀 클릭 후 Ctrl+V: 해당 위치부터 붙여넣기 | 표 바깥 Ctrl+V: 새 행 추가 — {pasteHint}
        </div>
      )}

      <div className={`sheet-wrap${isDragging ? ' is-dragging' : ''}`}>
        <table className="sheet-table" style={{ tableLayout: 'fixed', width: 'auto' }}>
          <thead>
            <tr>
              {isEditor && (
            ...
            {filteredRows.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS.length + (isEditor ? 2 : 0)} className="empty">
                  {searchQuery.trim() ? `"${searchQuery}" 검색 결과가 없습니다.` : `데이터가 없습니다.${isEditor ? ' + 행 추가 버튼을 누르거나 Ctrl+V로 붙여넣으세요.' : ''}`}
                </td>
              </tr>
              ...
              <tr key={row.id} className={selected.has(row.id) ? 'row-selected' : ''}>
                ...
                {COLUMNS.map((c, colIdx) => (
                  <td
                    ...
                    style={PRICE_KEYS.has(c.key) ? { textAlign: 'right' } : undefined}
                  >
                    <input
                      type="text"

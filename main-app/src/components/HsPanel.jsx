...
function mapToRows(map) {
  ...
  // 하나라도 _idx가 있으면 _idx 기준 정렬 (숫자형 키는 Object.entries가 자동 정렬되므로)
  const haIdx = entries.some(([, v]) => typeof v?._idx === 'number');
  if (haIdx) entries.sort((a, b) => (a[1]?._idx ?? Infinity) - (b[1]?._idx ?? Infinity));
  const rows = entries.map(([sku, v]) => ({
    _id: nextId(),
    sku,

...
import CompuzoneTab from './CompuzoneTab';
import AtozTab from './AtozTab';
import OrderConverter from './OrderConverter';
import MiracleTab from './MiracleTab';
...
export default function B2BPage({ user, hasPermission, onLogout }) {
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const isSettings = /^\/b2b-order\/settings(\/)?$/.test(window.location.pathname);
  const scrollActiveHash = useScrollActiveHash(!isSettings);
        ...
        <div className="main">
          <header className="topbar">
            ...
            <div className="topbar-right">
              <a href="/b2b-order/" className="b2b-settings-link">← B2B 발주</a>
            </div>
          </header>
          <main className="b2b-app-main"><SettingsEditor /></main>
        </div>
      </div>
    );
  }
    ...
    <div className="app-shell">
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="topbar-right">
            {isEditor && (
              <a href="/b2b-order/settings/" className="b2b-settings-link">⚙️ 공용 설정</a>
            )}
          </div>
        </header>
        <main className="b2b-app-main">
